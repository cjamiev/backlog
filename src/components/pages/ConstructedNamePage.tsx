...
const ConstructedNamePage: React.FC = () => {
  ...
  useEffect(() => {
    if (isBackendAvailable && isLoadingNames) {
      loadRecordsByType('constructed-names').then((records: Name[]) => {
        setNames(records);
        setIsLoadingNames(false);
      });
    }
    if (!isBackendAvailable && !isLoadingPing) {
      const savedNames = getRecordsFromStorage('constructed-names', [...fakeNames]);
      setNames(savedNames);
      setIsLoadingNames(false);
    }
  ...
  const handleSubmit = async (payload: Name[]) => {
    if (!isBackendAvailable && !isLoadingPing) {
      localStorage.setItem('constructed-names', JSON.stringify(payload));
    } else {
